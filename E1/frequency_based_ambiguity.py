...
def main(dataset,model,model_style,epochs,root_path,num_layers,filter_names,rate1,rate2,new_value):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    with open(root_path  +'/ACC.txt', 'a') as f:
        print(f'attack  accuracy: {test_acc:.2f}%\n')
...
if __name__ == "__main__":
    ...
    for r1, r2 in zip(rate1, rate2):
        for new_value in new_values:
            root_path = f'frequency_based_ambiguity/{dataset}_{model_style}_epochs{epochs}/{format_rate(r1)}_{format_rate(r2)}_{new_value}'
            ...
            main(dataset, model,model_style,epochs, root_path, num_layers, filter_names, r1, r2, new_value)
