...
def extract_and_dct_conv_filters(layer, output_file):
    ...
    with open(output_file, 'w') as f:
        for filter_idx in range(filters.shape[0]):
            ...
            dct_row = dct(filter_data, type=2, axis=2, norm='ortho')


            filter_dct= dct(dct_row, type=2, axis=1, norm='ortho')
...
def modify_and_save_dct_filters(dct_filters, output_file,rate1,rate2,new_value):
    ...
    with open(output_file, 'w') as f:
        for filter_idx, filter_dct in enumerate(dct_filters):
            ...
            threshold1 = (height + width - 2) * rate1
            threshold2 = (height + width - 2) * rate2
...
def modify_idct_filters(dct_filters, output_file):
    ...
    with open(output_file, 'w') as f:
        for idx, filter_dct in enumerate(dct_filters):
            dct_row = idct(filter_dct, type=2, axis=2, norm='ortho')

    
            inv_filter= idct(dct_row, type=2, axis=1, norm='ortho')
...
def main(dataset,model,epochs,root_path,num_layers,filter_names,rate1,rate2,new_value):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    for i, conv_layer in enumerate(conv_layers):
        ...
        distances1_2 =compare_cossim_dct_filters_long(dct_filters_list[i],new_dct_filters_list[i])
...
if __name__ == "__main__":
    ...
    for r1, r2 in zip(rate1, rate2):
        for new_value in new_values:
            root_path = f'2D/{dataset}_{model_style}_epochs{epochs}/{format_rate(r1)}_{format_rate(r2)}_{new_value}'
            ...
            main(dataset, model, epochs, root_path, num_layers, filter_names, r1, r2, new_value)
